...
export function ProductCard({
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-[28px] border border-[var(--border-strong)] bg-[rgba(13,14,26,0.92)] p-3 text-[var(--text-primary)] shadow-[var(--shadow-float)] transition duration-700 ease-out hover:[transform:perspective(1200px)_rotateX(0deg)] md:[transform:perspective(1200px)_rotateX(3deg)] ${className}`}
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(49,57,251,0.18),transparent_42%)]" />
      <div className="relative overflow-hidden rounded-[22px] border border-[var(--border-default)] bg-[var(--bg-base)]">
        <div className="flex items-center justify-between border-b border-[var(--border-default)] px-5 py-4">
          ...
          <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-[var(--text-inverse)]">
            {badge}
          </span>
        </div>

        <div className="grid gap-0 lg:grid-cols-[220px_1fr]">
          <aside className="hidden border-r border-[var(--border-default)] bg-[var(--bg-surface)] p-5 lg:block">
              ...
              {navItems.map((item, index) => (
                <div
                  ...
                  className={`rounded-2xl px-4 py-3 text-sm ${
                    index === 0 ? "bg-white text-[var(--text-inverse)]" : "bg-[var(--bg-glass)] text-[var(--text-secondary)]"
                  }`}
                >
                  {item}
                </div>
          ...
          <main className="min-h-[360px] p-5 md:p-7">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              ...
              <div className="rounded-full border border-[var(--border-brand)] bg-[rgba(200,168,255,0.12)] px-4 py-2 text-sm font-semibold text-[var(--brand-purple)]">
                4.9 ★
              </div>
            </div>
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {items.map((item) => (
                <div key={item.label} className="rounded-[20px] border border-[var(--border-default)] bg-[var(--bg-surface)] p-4">
                  <div
            ...
            <div className="mt-6 overflow-hidden rounded-[22px] border border-[var(--border-default)] bg-[rgba(255,255,255,0.04)] p-4">
              {variant === "tryon" ? (
