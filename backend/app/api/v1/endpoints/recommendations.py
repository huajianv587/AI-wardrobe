...
from app.schemas.recommendation import HomeTopThreeResponse, RecommendationRequest, RecommendationResponse
...
@router.get("/home-top3", response_model=HomeTopThreeResponse)
def home_top_three(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_or_demo_user),
) -> HomeTopThreeResponse:
    return assistant_service.generate_home_top_three(db, current_user)
